...
export class CacheService {
    ...
    static setPodcasts(podcasts: unknown) {
        StorageService.set(STORAGE_KEYS.PODCASTS, podcasts);
    }

    static getEpisodes(podcastId: string) {
        const allEpisodes = StorageService.get<Record<string, unknown>>(STORAGE_KEYS.EPISODES);
        return allEpisodes?.data[podcastId] || null;
    }

    static setEpisodes(podcastId: string, episodes: unknown) {
        const allEpisodes = StorageService.get<Record<string, unknown>>(STORAGE_KEYS.EPISODES)?.data || {};
