...
import { PodcastProvider, usePodcast, podcastReducer, initialState } from '../PodcastContext';
...
describe('PodcastContext', () => {
    ...
    test('PodcastProvider provides state and actions', () => {
        ...
        const sampleEpisode: Episode = { id: 'e1', title: 'Ep1', date: '2025-01-01', duration: '10:00', audioUrl: 'http://a' };

        const TestComp: React.FC = () => {
            const { podcasts, filteredPodcasts, filter, setPodcasts, setFilter, setEpisodes, episodes, setSelectedPodcast, selectedPodcast } = usePodcast();
            return (
                <div>
                    ...
                    <span data-testid="selected">{selectedPodcast ? selectedPodcast.id : 'null'}</span>

                    <button onClick={() => setPodcasts(samplePodcasts)}>set</button>
                    <button onClick={() => setFilter('Alice')}>filter</button>
                    <button onClick={() => setEpisodes([sampleEpisode])}>setEpisodes</button>
                    <button onClick={() => setSelectedPodcast({ ...samplePodcasts[0], episodeCount: 1 } as PodcastDetail)}>select</button>
                </div>
            );
        };
    ...
    test('podcastReducer handles actions correctly', () => {
        const s1 = podcastReducer(initialState, { type: 'SET_PODCASTS', payload: [{ id: '1', name: 'X', artist: 'A', image: 'i' }] });
        expect(s1.podcasts.length).toBe(1);

        const s2 = podcastReducer(s1, { type: 'SET_FILTER', payload: 'x' });
        expect(s2.filter).toBe('x');

        const ep: Episode = { id: 'e1', title: 't', date: 'd', duration: '1', audioUrl: 'u' };
        const s3 = podcastReducer(s2, { type: 'SET_EPISODES', payload: [ep] });
        expect(s3.episodes.length).toBe(1);

        const detail = { id: '1', name: 'X', artist: 'A', image: 'i', episodeCount: 1 } as PodcastDetail;
        const s4 = podcastReducer(s3, { type: 'SET_SELECTED_PODCAST', payload: detail });
